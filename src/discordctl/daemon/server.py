...
from discordctl.ops.registry import REGISTRY, BusContext, HandlerError, Registry
...
async def _handle_op(request: web.Request) -> web.Response:
    ...
    ctx = BusContext(
        bot=request.app[KEY_BOT],
        dry_run=effective_dry_run,
        confirm=payload.confirm,
        yes_really=payload.yes_really,
        actor="claude-code",
    ...
    await audit.record(
        ...
        args=payload.args,
        actor=ctx.actor,
        dry_run=effective_dry_run if mutating else False,
        confirm=payload.confirm,
        ok=ok,
        duration_ms=now_ms() - started,
        error=error,
