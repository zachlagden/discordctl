...
from claude_control.ops.handlers import category as cat_ops
from claude_control.ops.registry import BusContext


def make_guild():
    cat = SimpleNamespace(id=300, name="staff", position=0, channels=[],
                          edit=AsyncMock(), delete=AsyncMock())
    guild = SimpleNamespace(id=1, categories=[cat], channels=[cat],
