...
def make_role(id=10, name="mod", position=2):
    return SimpleNamespace(
        id=id, name=name, position=position, hoist=False, mentionable=True,
...
def make_member(id=100, name="alice", roles=None):
    return SimpleNamespace(
        id=id, name=name, display_name=name, nick=None, bot=False,
        roles=roles or [], joined_at=None,
...
def make_channel(id=200, name="general", type_name="text", position=0, category_id=None):
    return SimpleNamespace(
        ...
        category_id=category_id, topic=None, nsfw=False, slowmode_delay=0,
    )
...
def fake_ctx():
    return BusContext(
        bot=SimpleNamespace(),
        dry_run=True, confirm=False, yes_really=False, actor="test",
